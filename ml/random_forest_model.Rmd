...
```{r}
cv_samples <- vfold_cv(ref_train, strata = REF, v = 10, repeats = 10)
```
...
```{r}
ggsave("rf_vi.png", plot = rf_vi, dpi = 600, device = "png")
```
...
metrics_on_test %>%
  ...
  mutate(differences = train_estimate - test_estimate) %>% 
  rename(Metric = .metric, Test = test_estimate, Train = train_estimate, Differences = differences) %>% 
  flextable() -> train_met
train_met
```
...
rf_model_results <- tibble(truth = ref_binaric$REF,
...
```{r}
save_as_image(train_met, path = "train_met.png", expand = 10, res = 600)
save_as_image(fin_met, path = "fin_met.png", expand = 10, res = 600)
ggsave("roc_curve.png", plot = roc_curve, dpi = 600, device = "png")
```
